<?if(!defined("B_PROLOG_INCLUDED") || B_PROLOG_INCLUDED!==true)die();?><div class="section">
<!-- Всплывающий блок оплаты -->
    ...
    <div class="bx_section pay_section">
        <div class="pay-system-title"><?=GetMessage("PAY_SYSTEM")?></div>
        <div class="summ-to-pay"><?=GetMessage("SUM_PAY_SYSTEM").' '.round($arResult["ORDER_PRICE"], 0)?></div>
        <?
            //             arshow($arResult);
        ?>
        <?
            ...
            foreach($arResult["PAY_SYSTEM"] as $arPaySystem)
            {
                if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) > 0 || intval($arPaySystem["PRICE"]) > 0)
                {
                    ...
                    {
                    ?> 
                    <div class="bx_block w100 vertical pay-block">
                        <div class="bx_element">
                            <input type="radio"
                            ...
                            <label for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
                                ...
                                <?
                                    ...
                                    */
                                ?> 
                                <div class="check-input <?if ($arPaySystem["CHECKED"]=="Y" && !($arParams["ONLY_FULL_PAY_FROM_ACCOUNT"] == "Y" && $arResult["USER_VALS"]["PAY_CURRENT_ACCOUNT"]=="Y")) echo "checked-pay";?>"></div>
                                <div class="bx_description descr-pay">
                                    <?if ($arParams["SHOW_PAYMENT_SERVICES_NAMES"] != "N"):?>
                                        <div class="pay-type-title"><?=$arPaySystem["PSA_NAME"];?></div>
                                        <?endif;?>
                                    <? /*
                                        ?><p>
                                        <?
                    ...
                    <div class="bx_block w100 vertical pay-block">
                        <div class="bx_element">
                            ...
                            <label for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
                                ...
                                <?php if ($arPaySystem["ID"] == '12') { ?>
                                    <div class="tooltip-wrapper">
                                        <div class="tooltip tooltip--payment">
                                            <div class="tooltip__title">Доступные способы:</div>
                                        </div>
                                    </div>
                                <?php } ?>
                            </label>
                ...
                if (strlen(trim(str_replace("<br />", "", $arPaySystem["DESCRIPTION"]))) == 0 && intval($arPaySystem["PRICE"]) == 0)
                {
                    ...
                    {
                    ?>
                    <div class="bx_block w100 horizontal">
                        <div class="bx_element">
                    ...
                    <div class="bx_block w100 horizontal">
                        <div class="bx_element">
                            ...
                            <label for="ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>" onclick="BX('ID_PAY_SYSTEM_ID_<?=$arPaySystem["ID"]?>').checked=true;changePaySystem();">
                                ...
                                <?endif;?>
                                <?php echo '<!--', print_r($arPaySystem['ID']), '-->'; ?>
                            </label>
                        </div>
                    </div>
                    <?
