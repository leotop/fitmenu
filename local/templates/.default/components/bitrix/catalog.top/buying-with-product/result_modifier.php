<?
    ...
    /*$arSelect = Array();
    $arFilter = Array("ID"=>$arParams["PRODUCTS_WITH_BUYING_VALUE"]);
    $obItemsBuying = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
    while($arItemsBuying=$obItemsBuying->Fetch()) {
        arshow($arItemsBuying);
        $ar_res = CCatalogProduct::GetByIDEx($arItemsBuying["ID"]);
        arshow($ar_res);
    }*/
