...
<input     
    ...
    value="<? if (!empty($_POST["ORDER_PROP_CITY_val"])) { echo $_POST["ORDER_PROP_CITY_val"];} else { $arResult["LOCATION_STRING"]; }?>"
    class="search-suggest input-city" type="text"
    autocomplete="off"
    placeholder="<?=GetMessage("PH_REQUIRED");?>"
    onfocus="loc_sug_CheckThis(this, this.id);"
    <?=($arResult["SINGLE_CITY"] == "Y" ? " disabled" : "")?>/>
<input type="hidden" name="ORDER_PROP_5" id="ORDER_PROP_5" value="<? if (!empty($_POST["ORDER_PROP_CITY_val"])) { echo $_POST["ORDER_PROP_CITY_val"];} else { $arResult["LOCATION_STRING"]; }?>">
<input type="hidden" name="<?echo $arParams["CITY_INPUT_NAME"]?>" id="<?echo $arParams["CITY_INPUT_NAME"]?>" value="<? if (!empty($_POST["ORDER_PROP_CITY"])) { echo $_POST["ORDER_PROP_CITY"];} else { $arResult["LOCATION_VALUE"]; }?>">
