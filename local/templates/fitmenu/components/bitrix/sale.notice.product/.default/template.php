...
<?elseif ($arResult["STATUS"] == "N"):?>
	<span id="url_notify_<?=$arParams['NOTIFY_ID']?>">
        <a href="javascript:void(0)"  class="notify" onClick="notifyProduct('<?=$arResult["NOTIFY_URL"]?>', <?=$arParams['NOTIFY_ID']?>);"><?=GetMessage('NOTIFY');?></a>
    </span>
<?elseif ($arResult["STATUS"] == "R"):?>
	<span id="url_notify_<?=$arParams['NOTIFY_ID']?>">
		<a href="javascript:void(0)" class="notify" onClick="showNotify(<?=$arParams['NOTIFY_ID']?>)" id="notify_product_<?=$arParams['NOTIFY_ID']?>"><?=GetMessage('NOTIFY');?></a>
	</span>
<?endif;?>
...
<div id="popup_form_notify" style="display: none;">
	...
	<table border="0" cellpadding="4" cellspacing="0" width="300">
		...
		<tr>
			...
			<td valign="top">
				<input type="text" value="" name="popup_user_email" class="it-text" id="popup_user_email" size="25">
...
function notifyProduct(url, id)
{
	...
	BX.ajax.post(url, '', function(res) {
		...
		if (BX('url_notify_'+id)){
            ...
            BX('url_notify_'+id).className('sussess');
		}
