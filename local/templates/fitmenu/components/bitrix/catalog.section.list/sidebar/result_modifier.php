...
if (0 < $arResult['SECTIONS_COUNT'])
{
	...
	if ($boolPicture || $boolDescr)
	{
		...
		while ($arSection = $rsSections->GetNext())
		{
            ...
            /* LAST POST*/
            $arSelect = Array("ID", "NAME", "DATE_ACTIVE_FROM", "CREATED", "DETAIL_PAGE_URL", "PREVIEW_TEXT");
            $arFilter = Array("IBLOCK_ID"=>IntVal($arParams['IBLOCK_ID']), "SECTION_ID" => $arSection['ID'], "ACTIVE_DATE"=>"Y", "ACTIVE"=>"Y");
            $res = CIBlockElement::GetList(Array('CREATED' => 'desc'), $arFilter, false, Array("nPageSize"=>1), $arSelect);
            while($ob = $res->GetNextElement())
            {
             $LAST_POST = $ob->GetFields();
             $arResult['SECTIONS'][$key]['LAST_POST'] = $LAST_POST;
            }
		}
	}
}
