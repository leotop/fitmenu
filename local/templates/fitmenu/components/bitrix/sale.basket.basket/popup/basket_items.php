...
<div id="basket_items_list">
	<div class="bx_ordercart_order_table_container">
		<table id="basket_items">
			...
			//	print_r($arItem) ?>
					<tr id="<?=$arItem["ID"]?>" class="basket__main-row clearfix">
						...
						foreach ($arResult["GRID"]["HEADERS"] as $id => $arHeader):
							...
							?>
								...
								<td class="item">
									<span class="bx_ordercart_item title">
                                     <? // print_r($arItem) ?>
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?><a href="<?=$arItem["DETAIL_PAGE_URL"] ?>"><?endif;?>
											<?=$arItem["NAME"]?>
										<?if (strlen($arItem["DETAIL_PAGE_URL"]) > 0):?></a><?endif;?>
						...
						?>
							...
							<td class="control">
								...
								if ($bDeleteColumn):
								?>
									<a href="javascript:void(0)<?//?action=delete&id=<?//=$arItem["ID"]?><?//=str_replace("#ID#", $arItem["ID"], $arUrls["delete"])?>" 
                                       class="delete"
                                       onclick="delfrombasket(<?=$arItem['ID'];?>)">
                                       </a>
								<?
								endif;
								if ($bDelayColumn AND FALSE):
								?>
	...
	<div class="bx_ordercart_order_pay">
		...
		<div class="bx_ordercart_order_pay_right">
			<table class="bx_ordercart_order_sum">
				...
				<?endif;?>
					...
					<tr>
						<td class="custom_t1"><? echo GetMessage("TOTAL_SUMM") ?></td>
                    ...
                    <tr   class="sale">
						...
						<td class="fwb" id="allSum_FORMATED"><?=$arResult["allSum_FORMATED"]?></td>
					</tr>
                    <? if(! empty($arResult['DISCOUNT_PRICE_ALL'])): ?>
                    <tr class="discount">
						<td class="fwb"><?=GetMessage("SALE_DISCOUNT")?>:</td>
						<td class="fwb" id="allSum_FORMATED"><?= $arResult["DISCOUNT_PRICE_ALL_FORMATED"]; ?></td>
					</tr>
                    <? endif ?>
		...
		<div class="bx_ordercart_order_pay_center">
            ...
            <? $_page = 'http://'.$_SERVER['SERVER_NAME'].''.$APPLICATION->GetCurPage(); ?>
            <a href="<? echo (! empty($_SERVER['HTTP_REFERER']) ? ($_SERVER['HTTP_REFERER'] == $_page ? '/' : $_SERVER['HTTP_REFERER']) : '/' ) ?>" class="checkout l"><?=GetMessage("SITE_BACK")?></a>
		</div>
	</div>
</div>
<?
