    ...
    //arshow($arParams);
        if(!$isSearch)
            ...
            <?$APPLICATION->IncludeFile(SITE_TEMPLATE_PATH.'/includes/catalog/choose_nmg.php',array("arChoose"=>array(
                    ...
                    "2"=>array("NAME"=>"по цене(возр.)", "CODE"=> "CATALOG_PRICE_".$arResult["PRICES_ALLOW"][0], "sort"=>"ASC"),
                    "3"=>array("NAME"=>"по цене(убыв.)", "CODE"=> "CATALOG_PRICE_".$arResult["PRICES_ALLOW"][0], "sort"=>"DESC"),    
                    "4"=>array("NAME"=>"популярности(возр.)", "CODE" => "PROPERTY_rating", "sort"=>"ASC"),
                    "5"=>array("NAME"=>"популярности(убыв.)", "CODE" => "PROPERTY_rating", "sort"=>"DESC"),
                )));?><?
            }?>
        <div class="clear"></div>
    </div>
...
<table class="data-table category" cellspacing="0" cellpadding="0"  width="100%">
    ...
    <tr id="<?=$this->GetEditAreaId($arElement['ID']);?>">
        ...
        <td class="td hidden-480 <? echo ($arProperty['CODE'] == 'CML2_MANUFACTURER' ? "hidden" : "")  ?>">
        ...
        <?endforeach?>
        <?foreach($arResult["PRICES"] as $code=>$arPrice):?>
        <td>  <?//arshow($arElement["OFFERS"][0]["MIN_PRICE"])?>
                             ...
                             <div class="offer <? echo ($b_o > 0 ? 'hidden' : '');?>" id="_offer_<? echo $arShowOffer['ID']  ?>">
                                <?  if($arShowOffer['CATALOG_QUANTITY'] > 0): // В наличии ?>
                                    ...
                                    <div class="left">
                                        <? foreach($arShowOffer['ATTRIBUTES'] as $k => $CML2_ATTRIBUTE): ?>
                                            <p>
                                            <b><? echo '<span class="shide"><!--', base64_encode( 
                                                $arShowOffer['ATTRIBUTES_DECRIPTION'][$k]
                                            ), '--></span>: ';?></b>

                                            <? echo '<span class="shide"><!--', base64_encode( 
                                                $CML2_ATTRIBUTE 
                                            ), '--></span>';?>
