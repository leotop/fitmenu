...
foreach ($arResult["ELEMENTS_LIST"] as $key => $arItem)
{
    ...
    $gray_src = $arItem["PREVIEW_PICTURE"]["GRAY_SRC"];
...
?>
		<div id="photo_cont_<?=$arItem["ID"]?>" class=" <? echo ($index > 1 ? 'hidden ': '') ?>photo-item-cont <?if ($arParams["PERMISSION"] >= "X"){echo ' photo-item-cont-moder';}?>" title="<?= $arItem["TITLE"]?>">
			<a class="photo-item-inner" style="width: <?= $arParams['THUMBNAIL_SIZE']?>px; height: <?= $arParams['THUMBNAIL_SIZE']?>px; background: url(/<? echo $gray_src ?>);" href="<?=$arItem["URL"]?>" id="photo_<?=$arItem["ID"]?>">
