	...
	<table>
		...
		<?if(is_array($arResult["PREVIEW_PICTURE"]) || is_array($arResult["DETAIL_PICTURE"])):?>
			<?if(count($arResult["MORE_PHOTO"])>0):?>
				<td rowspan="2" style="width:450px;height: 450px;vertical-align:top;position:relative;">
			...
			<?else:?>
				<td rowspan="2" style="width:450px;vertical-align:top;position:relative;">
					<div style="position:relative;">
		...
		<table class="equipment" rules="rows">
			<thead>
				<tr>
					...
					<td><?if($arProp["NAME"]=="ДополнительноеПокрытие"):?>Доп. покрытие<?else:?><?if($arProp["NAME"]=="ОсновноеПокрытие"):?>Основное покрытие<?else:?><?=$arProp["NAME"]?><?endif?><?endif?></td>
					<?endforeach?>
