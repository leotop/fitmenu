...
?><? if($_SERVER['REMOTE_ADDR'] == '85.12.196.89') :
//    $res = CIBlockElement::GetByID($ElementID);
//    $ar_res = $res->GetNext();
//    $APPLICATION->SetPageProperty("title", 'Купить ' . $ar_res['NAME'] . ' по доступной цене в Екатеринбурге | Интернет магазин спортивного питания «Fitmenu»') ;
endif; ?><?
if (0 < $ElementID)
{
	$arRecomData = array();
	...
	if($arParams["USE_ALSO_BUY"] == "Y" && \Bitrix\Main\ModuleManager::isModuleInstalled("sale") && !empty($arRecomData))
	{
        ...
        ?>
        <? $APPLICATION->IncludeComponent("bitrix:sale.recommended.products", "sidebar", array(
			"ID" => $ElementID,
			"TEMPLATE_THEME" => (isset($arParams['TEMPLATE_THEME']) ? $arParams['TEMPLATE_THEME'] : ''),
