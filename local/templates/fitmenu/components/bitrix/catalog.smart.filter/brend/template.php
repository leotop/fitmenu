...
?>
<div class="menu-open__wrapper visible">
	<div class="menu-open__inner menu-open__inner--main">
  	<span class="menu-open__title">Выбор по параметрам</span>
  	<button class="menu-open__trigger" type="button">☰</button>
  </div>
	<div class="menu-open bx_filter_horizontal bx_<?=$arParams["TEMPLATE_THEME"]?>">
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smartfilter">
			...
			<?foreach($arResult["ITEMS"] as $key=>$arItem):?>
               ...
               ?>
				...
				<?$classstring = 'bx_filter_block f';
				$cellnum = 2;?>
                <?  if(count($arItem["VALUES"]) < 15):
                	$c_b =  round(count($arItem["VALUES"])/$cellnum); 
					else:
					$cellnum = 3;
                	$c_b =  round(count($arItem["VALUES"])/$cellnum); 
					$classstring = 'bx_filter_block th';
					endif;
                	?>
				<div class="bx_filter_container brendsWrapper active <?php 
    foreach($arrItem["VALUE"] as $item) { 
    	echo $item; 
    }
    ?>">
					<span class="bx_filter_container_title" onclick="hideFilterProps(this)"><?=$arItem["NAME"]?></span>
					<div class="wrapper_b active">
