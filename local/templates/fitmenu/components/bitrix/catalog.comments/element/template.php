...
if (!$templateData['BLOG']['BLOG_FROM_AJAX'])
{
	...
	if ($arParams["FB_USE"] == "Y")
	{
		...
		$arData["FB"] = array(
			...
			"CONTENT" => '
			<div id="bx-cat-soc-comments-fb_'.$arResult['ELEMENT']['ID'].'">
			<div class="fb-comments" data-version="v2.2" data-href="'.$arResult["URL_TO_COMMENT"].'"'.
			...
			(isset($arParams["WIDTH"]) ? ' data-width="'.$arParams["WIDTH"].'"' : '').
			'></div></div>'.PHP_EOL
		);
	}
